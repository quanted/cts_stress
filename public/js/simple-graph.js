...
SimpleGraph = function(elemid, options, data) {
  ...
  this.y = d3.scale.linear()
          .domain([0, d3.max(this.data, function(d) { return d.latency; })])
          .range([ this.cy, 0 ]);
  ...
  this.points = d3.range(self.data.length).map(function(i) {
    return {x: self.data[i]['x'], y: self.data[i]['latency']};
  });
  ...
  this.vis.append("svg").selectAll("circle")
    .data(this.data)
    .enter().append("circle")
      .attr("cx", function (d,i) { return self.x(d.x); })
      .attr("cy", function (d) { return self.y(d.latency); })
      .attr("r", 3)  // radius of dots for scatter plot
      .style("stroke", "none")
      .style("fill-opacity", 1.0)
      .style("fill", function (d, i) {
        // Color codes by calculator:
        if (d.calc == "chemaxon") {
          return "#c82300";
        }
        else if (d.calc == "epi") {
...
SimpleGraph.prototype.update = function() {
  ...
  // Sets any changes to perform during update (i.e., drag, zoom events):
